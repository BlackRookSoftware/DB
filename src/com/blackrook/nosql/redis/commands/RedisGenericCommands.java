...
public interface RedisGenericCommands
{
	 ...
	 * <p>Serialize the value stored at key in a Redis-specific format and return it to the user. The returned value can be synthesized back into a Redis key using the {@link #restore} command.</p>
	 ...
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p><b>expireat</b> has the same effect and semantic as {@link #expire}, but instead of specifying the number of seconds representing the TTL (time to live), it takes an absolute <a href="http://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> (seconds since January 1, 1970).</p>
	 * @param key the key to expire.
	 ...
	 * <p><strong>Available since 2.6.0.</strong></p>
	 * <p><strong>Time complexity:</strong> This command actually executes a {@link #dump} and {@link #del} in the source instance, and a {@link #restore} in the target instance. See the pages of these commands for time complexity. Also an O(N) data transfer between the two instances is performed.</p>
	 ...
	 * <p><strong>Available since 1.0.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Move <code>key</code> from the currently selected database (see {@link RedisStringCommands#select}) to the specified destination database. When <code>key</code> already exists in the destination database, or it does not exist in the source database, it does nothing. It is possible to use <a href="/commands/move">MOVE</a> as a locking primitive because of this.</p>
	 ...
	 * <p><strong>Time complexity:</strong> O(1).</p>
	 * <p>The <code>object</code> command allows to inspect the internals of Redis Objects associated with keys. It is useful for debugging or to understand if your keys are using the specially encoded data types to save space. Your application may also use the information reported by the <code>object</code> command to implement application level key eviction policies when using Redis as a Cache.</p>
	 ...
	 * @return the kind of internal representation used in order to store the value associated with a key, or null for missing key.
	 */
	 ...
	 * <p>The <code>object</code> command allows to inspect the internals of Redis Objects associated with keys. It is useful for debugging or to understand if your keys are using the specially encoded data types to save space. Your application may also use the information reported by the <code>object</code> command to implement application level key eviction policies when using Redis as a Cache.</p>
	 * @param the key to inspect.
	 * @return the number of seconds since the object stored at the specified key is idle (not requested by read or write operations). While the value is returned in seconds the actual resolution of this timer is 10 seconds, but may vary in future implementations.
	 */
	 ...
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Remove the existing timeout on <code>key</code>, turning the key from <em>volatile</em> (a key with an expire set) to <em>persistent</em> (a key that will never expire as no timeout is associated).</p>
	 ...
	 * <p><strong>Available since 2.6.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>This command works exactly like {@link #expire} but the time to live of the key is specified in milliseconds instead of seconds.</p>
	 ...
	 * <p><strong>Available since 2.6.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>PEXPIREAT has the same effect and semantic as {@link #expireat}, but the Unix time at which the key will expire is specified in milliseconds instead of seconds.</p>
	 * @param the key to expire.
	 * @param timestamp the timestamp in from-Epoch milliseconds.
	 * @return true if successful, false if not.
	 */
	 ...
	 * <p><strong>Available since 2.6.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Like {@link #ttl}, this command returns the remaining time to live of a key that has an expire set, with the sole difference that TTL returns the amount of remaining time in seconds while PTTL returns it in milliseconds.</p>
	 * @param the key to inspect.
	 ...
	 * <p><strong>Available since 1.0.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Renames <code>key</code> to <code>newkey</code>. It returns an error when the source and destination names are the same, or when <code>key</code> does not exist. If <code>newkey</code> already exists it is overwritten, when this happens {@link #rename} executes an implicit {@link #del} operation, so if the deleted key contains a very big value it may cause high latency even if {@link #rename} itself is usually a constant-time operation.</p>
	 ...
	 * <p><strong>Available since 1.0.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Renames <code>key</code> to <code>newkey</code> if <code>newkey</code> does not yet exist. It returns an error under the same conditions as {@link #rename}.</p>
	 * @param key the old name. 
	 * @param newkey the new name. 
	 * @return true if successful, false if not.
	 */
	 ...
	 * <p><strong>Available since 2.6.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1) to create the new key and additional O(N*M) to recostruct the serialized value, where N is the number of Redis objects composing the value and M their average size. For small string values the time complexity is thus O(1)+O(1*M) where M is small, so simply O(1). However for sorted set values the complexity is O(N*M*log(N)) because inserting values into sorted sets is O(log(N)).</p>
	 * <p>Create a key associated with a value that is obtained by deserializing the provided serialized value (obtained via {@link #dump}).</p>
	 ...
	 * <p>Returns or stores the elements contained in the list, set, or sorted set at <code>key</code>. By default, sorting is numeric and elements are compared by their value interpreted as double precision floating point number.
	 ...
	 * <p><strong>Available since 1.0.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Returns the remaining time to live of a key that has a timeout. This introspection capability allows a Redis client to check how many seconds a given key will continue to be part of the dataset.</p>
	 * @param the key to inspect.
	 ...
	 * <p><strong>Available since 1.0.0.</strong></p>
	 * <p><strong>Time complexity:</strong> O(1)</p>
	 * <p>Returns the string representation of the type of the value stored at <code>key</code>. The different types that can be returned are: <code>string</code>, <code>list</code>, <code>set</code>, <code>zset</code> and <code>hash</code>.</p>
